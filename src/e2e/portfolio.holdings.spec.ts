...
describe('Portfolio holdings section (HTTP mocks)', () => {
  let restore: () => void;
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('shows My Holdings section heading', async () => {
    render(PortfolioPage);
    expect(await screen.findByRole('heading', { name: /My Portfolio/i })).toBeInTheDocument();
    // Presence of holdings section heading (visible on both layouts)
    expect(await screen.findAllByText(/My Holdings/i)).toBeTruthy();
  }, 30000);
});