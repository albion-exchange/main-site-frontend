			...
			>
				...
				<div class="hidden sm:block">
					...
					<div class={assetStatsClasses}>
						<div class={statItemClasses}>
							...
							<div class={statValueClasses}>{item.asset.plannedProduction?.projections ? formatSmartNumber(item.asset.plannedProduction.projections.reduce((acc, curr) => acc + curr.production, 0), { threshold: 10000, decimals: 1, suffix: ' boe' }) : 'TBD'}</div>
						</div>
					</div>
				...
				<div class="sm:hidden">
					...
					<div class="text-sm text-black opacity-70">
						...
						<span>{item.asset.plannedProduction?.projections ? formatSmartNumber(item.asset.plannedProduction.projections.reduce((acc, curr) => acc + curr.production, 0), { threshold: 10000, decimals: 1, suffix: ' boe' }) : 'TBD'}</span>
					</div>
				</div>
							</div>
						</div>
