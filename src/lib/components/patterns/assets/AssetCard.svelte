	...
	<CardContent paddingClass="p-4 sm:p-6 lg:p-8 flex-1 flex flex-col">
		...
		<div class={highlightedStatsClasses}>
			<div class={highlightStatClasses}>
				<span class={highlightValueClasses}>{asset.plannedProduction?.projections ? formatSmartNumber(asset.plannedProduction.projections.reduce((acc, curr) => acc + curr.production, 0), { threshold: 10000, decimals: 1, suffix: ' boe' }) : 'TBD'}</span>
