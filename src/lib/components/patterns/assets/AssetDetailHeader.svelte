<script lang="ts">
	...
	import { getShareText, getShareUrls, shareViaWebAPI, copyToClipboard, getEmailContent, isMacOS, type ShareData } from '$lib/utils/sharing';
	...
	async function handleEmailShare() {
		...
		// On Mac, offer to copy email content to avoid the browser warning
		if (isMacOS()) {
			const confirmed = confirm('To avoid browser warnings, would you like to copy the email content to your clipboard instead? You can then paste it into your preferred email app.\n\nClick OK to copy, or Cancel to open your default email client.');
			
			if (confirmed) {
				try {
					await copyToClipboard(getEmailContent(shareData));
					...
					// Fall through to regular email sharing
				}
			}
		}
						...
						<div class="flex flex-col gap-4 lg:flex-row lg:justify-between lg:items-start lg:gap-4 mb-4">
							...
							<div class="hidden lg:flex lg:flex-col lg:items-end lg:gap-2 lg:flex-shrink-0">
								...
								<div class="flex gap-2">
									...
									<button class="flex items-center justify-center w-8 h-8 bg-white border border-light-gray text-black cursor-pointer transition-all duration-200 rounded hover:bg-light-gray hover:border-secondary hover:text-secondary" title="Share via email" aria-label="Share via email" on:click={handleEmailShare}>
										<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
