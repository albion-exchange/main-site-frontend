...
// Import all mock token metadata - consolidated imports
...
class DataStoreService {
  ...
  // Extracted method - builds reverse mapping
  ...
  // Extracted method - initializes token metadata from consolidated imports
  private initializeTokenMetadata(): void {
    this.assetMetadata = {};
    ALL_TOKEN_METADATA.forEach(metadata => {
      const converted = this.convertJsonToTokenMetadata(metadata as any);
      this.assetMetadata[converted.contractAddress] = converted;
    });
  }
  ...
  // Extracted helper for pricing formatting
  ...
  // Extracted helper for production units formatting
  private formatProductionUnits(units: any) {
    return {
