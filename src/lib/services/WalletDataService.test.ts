...
console.log(`Total Invested: ${walletDataService.formatCurrency(totalInvested)}`);
...
holdings.forEach(holding => {
  ...
  console.log(`  Invested: ${walletDataService.formatCurrency(holding.totalInvested)}`);
  console.log(`  Earned: ${walletDataService.formatCurrency(holding.totalEarned)}`);
  console.log(`  ROI: ${walletDataService.formatPercentage(holding.roi)}`);
  console.log(`  Unclaimed: ${walletDataService.formatCurrency(holding.unclaimedAmount)}`);
});
...
monthlyPayouts.forEach(payout => {
  console.log(`${payout.month}: ${walletDataService.formatCurrency(payout.totalPayout)}`);
});
...
allocation.forEach(token => {
  console.log(`${token.tokenSymbol}: ${token.tokensOwned} tokens (${walletDataService.formatPercentage(token.percentageOfPortfolio)} of portfolio)`);
});

// Example 7: Get wallet metrics
console.log('\n=== Wallet Metrics ===');
const metrics = walletDataService.getWalletMetrics();
console.log(`Total Portfolio Value: ${walletDataService.formatCurrency(metrics.totalValue)}`);
console.log(`Total ROI: ${walletDataService.formatPercentage(metrics.totalROI)}`);
console.log(`Average Monthly Income: ${walletDataService.formatCurrency(metrics.averageMonthlyIncome)}`);
console.log(`Portfolio Diversity Score: ${metrics.portfolioDiversity.toFixed(2)}`);
if (metrics.nextExpectedPayout) {
  console.log(`Next Expected Payout: ${walletDataService.formatCurrency(metrics.nextExpectedPayout.estimatedAmount)} on ${metrics.nextExpectedPayout.estimatedDate}`);
}
...
unclaimedByAsset.forEach(asset => {
  console.log(`\n${asset.assetName}:`);
  console.log(`  Total Unclaimed: ${walletDataService.formatCurrency(asset.totalUnclaimed)}`);
  asset.unclaimedPayouts.forEach(payout => {
    console.log(`  - ${payout.month}: ${walletDataService.formatCurrency(payout.amount)}`);
  });
});
...
performance.forEach((asset, index) => {
  ...
  console.log(`   Total Earned: ${walletDataService.formatCurrency(asset.totalEarned)}`);
  console.log(`   Avg Monthly: ${walletDataService.formatCurrency(asset.averageMonthlyPayout)}`);
});

// Example 10: Get estimated annual income
console.log('\n=== Estimated Annual Income ===');
const annualIncome = walletDataService.getEstimatedAnnualIncome();
console.log(`Estimated Annual Income: ${walletDataService.formatCurrency(annualIncome)}`);

// Example 11: Get payout frequency analysis
console.log('\n=== Payout Frequency Analysis ===');
const frequency = walletDataService.getPayoutFrequency();
console.log(`Average Days Between Payouts: ${frequency.averageDaysBetweenPayouts}`);
console.log(`Payout Consistency: ${frequency.isConsistent ? 'Consistent' : 'Variable'}`);
...
detailedPayouts.slice(-3).forEach(month => {
  ...
  month.assetBreakdown.forEach(asset => {
    console.log(`  - ${asset.assetName}: ${walletDataService.formatCurrency(asset.amount)} (${asset.status})`);
  });
});