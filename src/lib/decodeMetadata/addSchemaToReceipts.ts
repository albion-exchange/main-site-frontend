...
export function generateTokenInstanceFromSft(sft: OffchainAssetReceiptVault, sftMaxSharesSupply: string): Token{
	...
	const tokenInstance: Token = {
		...
		sharePercentage: undefined, // Unclear what this is yet.
...
export function generateAssetInstanceFromSftMeta(sft: OffchainAssetReceiptVault, pinnedMetadata: any, ): Asset {
	const assetInstance: Asset = {
		...
		coverImage: `${PINATA_GATEWAY}/${pinnedMetadata.cover}`,
		images: pinnedMetadata.gallery.map((image: string) => ({
			title: image,
			url: `${PINATA_GATEWAY}/${image}`,
			caption: ''
		})),
		location: {
		...
		assetTerms: {
			...
			amountTooltip: pinnedMetadata.asset.assetTerms.amountTooltip,
			paymentFrequency: pinnedMetadata.asset.assetTerms.paymentFrequency
		},
		tokenContracts: [sft.id],
		monthlyReports: pinnedMetadata.monthlyData.map((report: any) => ({
			...
			payoutPerToken: report.tokenPayout.payoutPerToken, // USD per token (optional for royalty assets)
		})),
		metadata: {
			createdAt: new Date(Number(sft.deployTimestamp) * 1000).toISOString() as ISODateTimeString,
			updatedAt: new Date(Number(sft.deployTimestamp) * 1000).toISOString() as ISODateTimeString
