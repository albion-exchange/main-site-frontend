...
export function useChartData() {
  ...
  function getProductionChartData(
    ...
    const data = reports.map((report, index) => {
      ...
      if (options.includeMovingAverage && index >= (options.movingAveragePeriod || 3) - 1) {
   ...
   */
  function getFinancialChartData(reports: MonthlyReport[]): MultiSeriesDataPoint[] {
    if (!reports?.length) return [];
   ...
   */
  function getCumulativeProductionData(reports: MonthlyReport[]): ChartDataPoint[] {
    if (!reports?.length) return [];
  ...
  function getEfficiencyChartData(
    ...
    return reports.map(report => {
      const revenuePerBarrel = report.production > 0 
        ? (report.revenue || 0) / report.production 
        : 0;
