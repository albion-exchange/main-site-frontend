...
export function useClaimsData() {
  ...
  function loadClaimsData() {
    ...
    try {
      ...
      state.update(s => ({
        ...s,
        error: error instanceof Error ? error.message : 'Failed to load claims data',
        loading: false
      }));
    }
  }
  ...
  const claimStats = derived(state, $state => ({
    avgClaimSize: $state.claimHistory.length > 0
      ? $state.totalClaimed / $state.claimHistory.length
      : 0,
    totalClaims: $state.claimHistory.length,
    lastClaimDate: $state.claimHistory.length > 0
      ? $state.claimHistory[0].date
      : null
  }));
