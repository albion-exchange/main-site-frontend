...
export function groupSftsByEnergyField(
	...
	tokensWithAssets.forEach(item => {
		const matchingField = ENERGY_FEILDS.find(field => 
			field.sftTokens.some(tokenAddress => 
				tokenAddress.toLowerCase() === item.token.contractAddress.toLowerCase()
			)
		);
		
		if (matchingField) {
			if (!grouped[matchingField.name]) {
				const fieldId = matchingField.name.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '');
			...
			if (!grouped[defaultGroupName]) {
				const fieldId = defaultGroupName.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '');
	...
	Object.values(grouped).forEach(group => {
		if (group.tokens.length > 0) {
			...
			const latestTokenWithAsset = tokensWithAssets.find(item => 
				item.token.contractAddress.toLowerCase() === latestToken.contractAddress.toLowerCase()
			);
