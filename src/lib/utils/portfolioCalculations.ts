 ...
 */
export function calculateUnclaimedPayout(userAddress: string, assetId: string): number {
...
export function calculateTotalUnclaimedPayout(userAddress: string): number {
  ...
  return holdings.reduce((total, holding) => total + holding.unclaimedAmount, 0);
}