...
const CONTEXT_EVENT_TOPIC = "0x17a5c0f3785132a57703932032f6863e7920434150aa1dc940e567b440fdce1f";
...
type OrderV3Type = [string, [string, string, string], [string, number, string][], [string, number, string][], string];
...
export async function sortClaimsData(csvClaims: any[], trades: any[], ownerAddress: string, feildName: string) {
    ...
    }).filter(log => log.address !== "0x0000000000000000000000000000000000000000");

    // Filter by owner address (required parameter)
    const normalizedOwnerAddress = ownerAddress.toLowerCase();
    
    // Filter CSV claims by owner address
    const filteredCsvClaims = csvClaims.filter(claim => 
        claim.address.toLowerCase() === normalizedOwnerAddress
    );
    
    // Filter decoded logs by owner address
    const filteredDecodedLogs = decodedLogs.filter(log => 
        log.address.toLowerCase() === normalizedOwnerAddress
    );

    // Filter into claimed and unclaimed arrays
    const { claimedCsv, unclaimedCsv } = filterClaimedAndUnclaimed(filteredCsvClaims, filteredDecodedLogs);
    
    // Calculate total amounts
    const totalClaimedAmount = claimedCsv.reduce((sum, claim) => 
        sum + BigInt(claim.amount), BigInt(0)
    );
    
    const totalUnclaimedAmount = unclaimedCsv.reduce((sum, claim) => 
        sum + BigInt(claim.amount), BigInt(0)
    );
    ...
    const claims: ClaimHistory[] = claimedCsv.map(claim => {
        ...
        const originalLog = logs.find(log => {
            const decodedData = decodeLogData(log.data);
            return decodedData.index === claim.index && decodedData.address === claim.address;
        });
...
export function getBlockRangeFromTrades(trades: any[]): { lowest: number; highest: number } {
...
function filterClaimedAndUnclaimed(csvClaims: any[], decodedLogs: any[]): {
    claimedCsv: any[];
    unclaimedCsv: any[];
} {
...
export function getMerkleTree(csvInput: any[]) {
    ...
    const leaves = csvInput.map((row) => {
        ...
        const address = row.address || row.Address || '0x0000000000000000000000000000000000000000';
 ...
 */
export function signContext(context: Array<string | bigint | number>): SignedContextV1Type {
    const wallet = Wallet.createRandom();

    const signer = wallet.address;

    // 2. Encode uint256[] context as tightly packed bytes (like abi.encodePacked in Solidity)
    const contextBytes = concat(context.map((n, index) => {
        // Always convert to wei for amounts (assuming index 1 is the amount)
        const value = typeof n === 'string' && n.includes('.') ? parseEther(n).toString() : n.toString();
    ...
    const signatureBytes = concat([getBytes(signature.r), getBytes(signature.s), Uint8Array.from([signature.v])]);
