...
export const createDataError = (message: string, context?: Record<string, any>, cause?: Error) => 
  new AppError(message, ErrorCode.DATA_LOAD_FAILED, ErrorSeverity.MEDIUM, context, cause);

export const createNotFoundError = (resource: string, id: string) => 
  new AppError(
    `${resource} not found`, 
    resource.toLowerCase().includes('asset') ? ErrorCode.ASSET_NOT_FOUND : ErrorCode.TOKEN_NOT_FOUND,
    ErrorSeverity.LOW,
    { resource, id }
  );
...
export const createServiceError = (service: string, operation: string, cause?: Error) =>
  new AppError(
